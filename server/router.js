...
/* 
  ...
  如60. "2 days". "10h". "7d". Expiration time.过期时间
   jwt.sign({},'秘钥'，'过期时间，{expiresTn:20*1,'1day''1h'}')
*/
...
router.get('/backend/item/insertTbItem', (req, res) => {
  ...
  */
  ...
  var arr = [title, image, sellPoint, price, cid, num, descs, category]
  console.log('开始传数据')
  sqlFn(sql, arr, result => {
    if (result.affectedRows > 0) {
      res.send({
...
/* 
  商品删除接口id
*/
router.get('/backend/item/deteItemById', (req, res) => {
  var id = req.query.id
...
router.get('/backend/item/updateTbItem', (req, res) => {
  ...
  var image = req.query.image || ''
  //下面没有删除category
  var sql = 'update project set title=?,sellPoint=?,price=?,category=?,cid=?,num=?,descs=?,image=? where id=?'
  var arr = [title, sellPoint, price, cid, category, num, descs, image, id]
  sqlFn(sql, arr, result => {
    if (result.affectedRows > 0) {
      res.send({
        status: 200,
